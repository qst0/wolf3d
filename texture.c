...
/*   Updated: 2017/01/24 20:36:11 by myoung           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	v->texture = malloc(sizeof(int*) * GEN_TEX_COUNT);
	v->tex_width = TEX_WIDTH;
	v->tex_height = TEX_HEIGHT;
	
	//Make space for all the textures
	for(int i = 0; i < GEN_TEX_COUNT; i++)
	{
		v->texture[i] = malloc(sizeof(int) * v->tex_width * v->tex_height);
	}

	//manually generate some textures with a loop
